...
class PerformanceDataHandler:
    ...
    def set_monitor(self, monitor):
        ...
        try:
            ...
            if self.monitor is None:
                raise ValueError(f"No class matching pattern '{pattern}' found in module '{self.monitor_module.__name__}'")
    ...
    def parse_perfdata_from_stdout(self, stdout_data_node):
        ...
        for stdout_data in stdout_data_node:
            ...
            performance_data_nodes.append(
                [cpu_util, mem_util, io_ops_read, io_ops_write, io_bytes_read, io_bytes_write, gpu_util, gpu_mem])

        # compute the mean, max, min across the nodes using pure mem_util lists, the mean, max and min lists of the other sensors
        performance_data_nodes.append(compute_mean_across_nodes("CPU", "MEANS", metrics_across_nodes))
        performance_data_nodes.append(compute_mean_across_nodes("MEM", "MEANS", metrics_across_nodes))
        performance_data_nodes.append(compute_mean_across_nodes("IO_OPS_READ", "MEANS", metrics_across_nodes))
        performance_data_nodes.append(compute_mean_across_nodes("IO_OPS_WRITE", "MEANS", metrics_across_nodes))
        performance_data_nodes.append(compute_mean_across_nodes("IO_BYTES_READ", "MEANS", metrics_across_nodes))
        performance_data_nodes.append(compute_mean_across_nodes("IO_BYTES_WRITE", "MEANS", metrics_across_nodes))
        performance_data_nodes.append(compute_mean_across_nodes("GPU_UTIL", "MEANS", metrics_across_nodes))
    ...
    def end_perfmonitor(self, code):
        ...
        performance_data_nodes = self.parse_perfdata_from_stdout(stdout_data_node)
