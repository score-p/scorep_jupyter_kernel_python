...
from parallel_marshall.balancedDistributionIterator import BalancedDistributionIterator
import importlib
import logging

# mode is automatically determined by the file object that is passed for dumping
mode = ""
backend = str(os.environ.get("PARALLEL_MARSHALL_BACKEND", 'dill'))
workers = int(os.environ.get("PARALLEL_MARSHALL_NWORKERS", 4))
...
def dump(obj, file):
    global mode
    if workers == 1:
        serializer_backend.dump(obj, file)
        return
    ...
    for subobj in BalancedDistributionIterator(obj, workers):
        subobj_path = paths[i]
        process = multiprocessing.Process(target=dump_subobj, args=(subobj, subobj_path))
    ...
    for subdict_path in paths:
        ...
        with os.fdopen(os.open(subdict_path, os.O_RDONLY), 'rb') as f:
            ...
            if isinstance(obj, dict):
                ...
                data_ = []
                data_.extend(obj)
            logger.debug(f"Loader loaded subdict")
